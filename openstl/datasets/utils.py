...
class PrefetchLoader:
    """Async pre-fetching DataLoader wrapper.

    On CUDA: uses a dedicated CUDA stream for async host→device transfers and
    optional FP16 normalisation — identical to the original behaviour.

    On MPS (Apple Silicon) or CPU: the CUDA Stream API is not available, so
    we fall back to a simple synchronous iterator that still applies the same
    normalisation / FP16 conversion logic.  The throughput difference is
    negligible on M-series chips because unified memory means there is no
    explicit host→device copy anyway.
    """
    ...
    def __init__(self,
        ...
        self.fp16 = fp16
        self.loader = loader
        self._use_cuda = torch.cuda.is_available()   # False on MPS / CPU
        ...
        if mean is not None and std is not None:
            ...
            if self._use_cuda:
                ...
                # Keep stats on CPU; Lightning will move batches to the right
                # device (mps / cpu) during the forward pass.
    ...
    # ------------------------------------------------------------------
    # Iteration — two paths depending on backend
    # ------------------------------------------------------------------

    def __iter__(self):
        if self._use_cuda:
            yield from self._iter_cuda()
        else:
            yield from self._iter_cpu()

    def _iter_cuda(self):
        """Original async CUDA-stream path (unchanged logic)."""
    ...
    def _iter_cpu(self):
        """Synchronous fallback for MPS / CPU — no CUDA Streams needed."""
        for batch_input, batch_target in self.loader:
            if self.fp16:
                if self.mean is not None:
                    batch_input  = batch_input.half().sub_(self.mean).div_(self.std)
                    batch_target = batch_target.half().sub_(self.mean).div_(self.std)
                else:
                    batch_input  = batch_input.half()
                    batch_target = batch_target.half()
            else:
                if self.mean is not None:
                    batch_input  = batch_input.float().sub_(self.mean).div_(self.std)
                    batch_target = batch_target.float().sub_(self.mean).div_(self.std)
                else:
