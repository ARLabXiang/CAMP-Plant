...
def load_data(dataname, batch_size, val_batch_size, num_workers, data_root, dist=False, **kwargs):
    ...
    if dataname == 'bair':
        ...
        return load_data(batch_size, val_batch_size, data_root, num_workers, **cfg_dataloader)
    elif dataname in ['komatsuna', 'arabidopsis']:
        # Plant growth datasets (KOMATSUNA and Arabidopsis/RoAD)
        from .dataloader_plant import load_data
        return load_data(dataname, batch_size, val_batch_size, data_root,
                         num_workers, **cfg_dataloader)
    else:
