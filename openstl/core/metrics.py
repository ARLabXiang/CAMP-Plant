...
class LPIPS(torch.nn.Module):
    ...
    def __init__(self, net='alex', use_gpu=True):
        ...
        # Select the best available accelerator instead of hardcoding CUDA
        if use_gpu:
        ...
        self.use_gpu = self._device.type != 'cpu'
    ...
    if 'ssim' in metrics:
        ...
        for b in range(pred.shape[0]):
            for f in range(pred.shape[1]):
                # channel_axis=-1 replaces the removed multichannel=True (scikit-image >= 0.19)
