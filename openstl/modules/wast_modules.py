...
class HighFocalFrequencyLoss(nn.Module):
    ...
    def __init__(self, loss_weight=0.001, level=1, tau=0.1, alpha=1.0, patch_factor=1, ave_spectrum=False, log_matrix=True, batch_matrix=False):
        ...
        self.level = level
        self.tau = tau
        # Do not call .cuda() here — Lightning moves the whole module to the
        # correct device (cuda / mps / cpu) via .to(device) at training start.
    ...
    def build_freq_mask(self, shape):
        ...
        # Create the mask on the same device as X (works for cuda / mps / cpu)
