...
def collect_env():
    ...
    env_info['MPS available (Apple Silicon)'] = mps_available
    ...
    if cuda_available:
        ...
        # On Apple Silicon, report the chip via sysctl
        try:
            chip = subprocess.check_output(
                'sysctl -n machdep.cpu.brand_string', shell=True)
            env_info['Apple chip'] = chip.decode('utf-8').strip()
        except subprocess.SubprocessError:
            env_info['Apple chip'] = 'Unknown'

    try:
        gcc = subprocess.check_output('gcc --version | head -n1', shell=True)
        env_info['GCC'] = gcc.decode('utf-8').strip()
    except subprocess.SubprocessError:
...
def measure_throughput(model, input_dummy):
    """Measure model inference throughput (samples / second).

    Uses CUDA Events for precise GPU timing on NVIDIA hardware.
    Falls back to time.perf_counter() on MPS (Apple Silicon) and CPU,
    which is accurate enough for a throughput estimate.
    """
    import time
    ...
    # Detect which timing method to use
    _device_type = (
        input_dummy[0] if isinstance(input_dummy, tuple) else input_dummy
    ).device.type
    use_cuda_events = (_device_type == 'cuda')

    total_time = 0
    with torch.no_grad():
        for _ in range(repetitions):
            if use_cuda_events:
                # High-resolution CUDA event timing (original path)
                ...
                curr_time = starter.elapsed_time(ender) / 1000   # ms → s
            else:
                # Wall-clock timing for MPS / CPU
