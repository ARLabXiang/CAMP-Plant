...
def _auto_device() -> str:
    ...
    On Apple Silicon (M1/M2/M3/M4) this returns 'mps' so training runs on
    the GPU cores via the Metal backend without any extra flags.
    """
...
def create_parser():
    ...
    # Set-up parameters
    parser.add_argument('--device', default=_auto_device(), type=str,
                        choices=['cuda', 'mps', 'cpu'],
                        help='Device for tensor computations: cuda | mps (Apple Silicon) | cpu. '
    ...
    parser.add_argument('--dataname', '-d', default='mmnist', type=str,
                        choices=['bair', 'mfmnist', 'mmnist', 'mmnist_cifar', 'noisymmnist', 'taxibj', 'human',
                                ...
                                # Plant growth datasets (CAMP paper)
                                'komatsuna', 'arabidopsis'],
    ...
    parser.add_argument('--method', '-m', default='SimVP', type=str,
                        choices=['ConvLSTM', 'convlstm', 'E3DLSTM', 'e3dlstm', 'MAU', 'mau', 'MIM', 'mim',
                                ...
                                # CAMP full model and ablation study variants
                                'camp',
                                'camp_base',    # backbone only (MSE)
                                'camp_no_cls',  # no classification (MSE + ExGI)
                                'camp_full',    # all components (MSE + BCE + ExGI)
                                'tau_full',     # TAU backbone + ExGI + cls head (translator-tap)
                                'tau_predcls',  # TAU backbone + ExGI + late-fusion cls (pred_y-tap)
                                'simvp_full',   # SimVP backbone + ExGI + cls head
                                'simvp_predcls',# SimVP backbone + ExGI + late-fusion cls
                                'mim_full',     # MIM backbone + ExGI + cls head
                                # Plant-dataset wrappers for baseline methods
                                'plant_convlstm', 'plant_predrnn', 'plant_phydnet',
                                'plant_simvp', 'plant_tau', 'plant_mim'],
...
def default_parser():
    default_values = {
        # Set-up parameters
        'device': _auto_device(),   # mps on Apple Silicon, cuda on NVIDIA, cpu otherwise
        'dist': False,
        'res_dir': 'work_dirs',
        'ex_name': 'Debug',
        'fp16': False,
